...
(use-package tex
  ...
  :init
  ;; Ask user for permission to save files before starting TeX.
  (setq TeX-save-query nil)

  ;; Enable TeX-source-correlate-mode (forward and inverse search minor mode).
  ...
  ;; Math mode delimiters inserted on pressing `$' by TeX-insert-dollar.
  (setq TeX-electric-math (cons "\\(" "\\)"))

  ;; Don't indent after open brace.
  (setq TeX-brace-indent-level 0)

  ;; Don't indent after a \left macro.
...
(use-package evil-tex-ts
  ...
  ;; NOTE: THIS NEEDS TO HAVE THE COMPILED libtree-sitter-latex.so!
  ;; In the repo, run BOTH `tree-sitter generate' and `make'.
